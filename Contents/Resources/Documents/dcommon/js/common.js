  ...
  if (document.getElementById("searchbody") === undefined || document.getElementById("searchbody") === null) {
    ...
    if (cat === null || (cat !== undefined && cat !== null && cat.innerHTML.indexOf("Cloud") === -1)) {
      for (i = 0; i < aList.length; i += 1) {
        ...
        setBaseUrl();
        if (aList[i].href && r.test(aList[i].href) && aList[i].href.indexOf("youtube.") === -1 && !aList[i].getAttribute("target") && ((classInfo !== '' && classInfo.indexOf("newwindow") === -1 && classInfo.indexOf("various") === -1) || classInfo === '') && !((aList[i].parentNode.parentNode.parentNode.getAttribute("class") && aList[i].parentNode.parentNode.parentNode.className.indexOf("footer-list") !== -1)) && aList[i].href.indexOf("/pls/topic") === -1) {
